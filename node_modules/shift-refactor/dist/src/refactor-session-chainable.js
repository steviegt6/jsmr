...
exports.refactor = exports.RefactorSessionChainable = void 0;
const global_state_1 = require("./global-state");
const query_1 = require("./misc/query");
const util_1 = require("./misc/util");
const refactor_session_1 = require("./refactor-session");
/**
 ...
 * @public
 */
class RefactorSessionChainable {
    constructor(session) {
    ...
    static with(plugin) {
        ...
        const BaseWithPlugins = (_a = class extends this {
            ...
            _a.with = RefactorSessionChainable.with,
            _a);
    ...
    first(selector) {
        if (selector) {
            return this.find(node => query_1.matches(node, selector)).first();
        }
     ...
     */
    references() {
        return this.nodes.flatMap(node => this.session.globalSession.findReferences(node));
    }
    /**
     ...
     */
    declarations() {
        return this.nodes.flatMap(node => this.session.globalSession.findDeclarations(node));
    }
    /**
     ...
     */
    statements() {
        const statements = this.nodes
            .map(util_1.innerBodyStatements)
            .filter(util_1.isNodeWithStatements)
            .flatMap((node) => node.statements);
     ...
     */
    lookupVariable() {
        return this.nodes.map(node => this.session.globalSession.lookupVariable(node));
    }
    /**
...
exports.RefactorSessionChainable = RefactorSessionChainable;
RefactorSessionChainable.plugins = [];
/**
 ...
 * @public
 */
function refactor(input, ...plugins) {
    const globalSession = new global_state_1.GlobalState(input);
    const refactorSession = new refactor_session_1.RefactorSession(globalSession.root, globalSession);
    if (plugins)
        return plugins
...
//# sourceMappingURL=refactor-session-chainable.js.map